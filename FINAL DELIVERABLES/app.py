...
app.config['uid'] = 'jlm87173'
app.config['pwd'] = 'NXflU8HNwZLcF7T2'
app.config['security'] = 'SSL'
try:
    mysql = DB2(app)

    conn_str='database=bludb;hostname=b70af05b-76e4-4bca-a1f5-23dbb4c6a74e.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;port=32716;protocol=tcpip;\
            uid=jlm87173;pwd=NXflU8HNwZLcF7T2;security=SSL'
...
def register():
    ...
    if request.method == 'POST' :
        ...
        if account:
            ...
            sql2 = "INSERT INTO USER (username, email,password) VALUES (?, ?, ?)"
...
def login():
    ...
    if request.method == 'POST' :
        ...
        param = "SELECT * FROM USER WHERE username = " + "\'" + username + "\'" + " and password = " + "\'" + password + "\'"
        res = ibm_db.exec_immediate(ibm_db_conn, param)
        dictionary = ibm_db.fetch_assoc(res)
